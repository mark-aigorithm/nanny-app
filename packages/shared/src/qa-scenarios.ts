...
const TIER_B: QaScenario[] = [
  {
    ...
    surface: 'Parent app',
    priority: 'P0',
    title: 'Mother registration — the phone code comes first',
    preconditions: ['A phone that can receive SMS and is not already registered'],
    steps: [
      'On the sign-in screen (Welcome to NannyNow) tap "Sign up" and choose Mother',
      'Enter a number that already has an account and tap "Send code"',
      'Enter her own number, request the code, try a wrong code first, then the right one',
      'Use "Change number" once, then come back to her number',
    ],
    expected: [
      'The step counter reads "step 1 of 5"',
      'A number that already has an account is refused with a Sign in button, and no SMS is sent',
      'The SMS arrives at the number entered, and a wrong code is refused',
      'The correct code moves on to "About you" (step 2 of 5)',
    ],
  },
  {
    ...
    preconditions: ['On "About you", with her number verified'],
    steps: [
      'Tap Continue with everything empty',
  ...
  },
  {
    ...
    preconditions: ['On "Secure your account", with access to that inbox'],
    steps: [
    ...
    expected: [
      'The email arrives at the address typed on "About you"',
      'A wrong code is refused, and the password fields stay hidden',
      'Resend is disabled for its cooldown then works',
      'The correct code shows the address as verified and reveals the password fields on the same screen',
    ],
  },
  {
    ...
    preconditions: ['On "Secure your account", after the email code'],
    steps: [
      'Type a short password, then one with no uppercase, then one with no digit',
      'Type a valid password and confirm it, continue, then go Back and forward again',
    ],
    expected: [
      ...
      'Going Back keeps the password she typed',
    ],
  },
  {
    ...
    surface: 'Parent app',
    priority: 'P0',
    title: 'Mother registration — location and preferences, then finish',
    ...
    steps: [
      ...
      'Continue, open "Terms of Service" and "Privacy Policy" on the last step, then accept and complete setup',
      'Press Back on the notification permission screen',
    ],
    expected: [
      'Address search returns suggestions and picking one moves the map',
      'Nothing is pre-selected under "What matters most?"',
      'Continue with the pin set but the street line empty asks for a street address, under that field',
      'The last step reads "step 5 of 5" and shows the number she is signed in as',
      'The Terms and Privacy Policy show the text operators wrote in the console (Settings → Legal documents)',
      'The account is created and the app moves on to the notification permission screen, then Home; Back never returns to the wizard',
      'Signing out and back in with that phone and password works, and her photo shows on Account details',
    ],
  },
  {
  ...
  },
  {
    ...
    expected: [
      'The mother\'s path is 5 steps; the nanny\'s is 7, because of her professional details and the ID',
      'Every screen\'s counter matches its place in the path, ending on "step n of n"',
  ...
  },
  {
    ...
    steps: [
      'Type details into "About you" as a Mother',
      'Go back to role selection and choose Nanny instead',
      'Look at "About you" again',
    ],
    expected: [
  ...
  },
  {
    ...
    preconditions: ['Registering as a nanny, past "Secure your account"'],
    ...
    expected: [
      ...
      'Continue is blocked until a location is confirmed; a missing street address is flagged under its field',
    ],
  },
  {
  ...
  },
  {
    ...
    steps: [
      ...
      'Set one working day to end before it starts',
      'Fix it and continue',
    ],
    expected: [
  ...
  },
  {
    ...
    surface: 'Nanny app',
    priority: 'P1',
    title: 'The nanny\'s profile is edited by an operator and shows on her Profile tab',
    preconditions: ['An approved nanny', 'An operator with the Nannies section'],
    steps: [
