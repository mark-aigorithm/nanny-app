...
- [ ] **Step 2: Result screen** — mirror `BookingPaymentResultScreen.tsx` exactly: on landing, call the **sync** mutation once (`POST /packages/purchases/:id/sync`, exposed by backend plan Task 7 — add a `useSyncPackagePayment` mutation alongside the Task 1 hooks), then poll `usePackageHours()` via `refetch`. Resolve **success** when the purchase's bucket reports `status === 'ACTIVE'`; **failure** when the synced payment reports `FAILED`. Show a success card ("50 hours added — valid until …") with "View my hours" → `router.replace('/(parent)/package-hours')`; on failure a retry button back to `checkout`. Reuse the 12s/10s pending/stalled timeout guards so the user is never trapped.
> Do NOT rely on the webhook alone, and do not infer success from "`availableHours` went up" — that races with any other credit. The sync call is what makes settlement deterministic from the client's side, exactly as the booking flow does it.
