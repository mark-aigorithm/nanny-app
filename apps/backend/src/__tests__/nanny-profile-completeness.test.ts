...
describe('getMissingNannyProfileFields', () => {
  it('returns no missing fields when all three are present', () => {
  ...
  it('returns every field, in constant order, when nothing is set', () => {
    ...
    expect(missing.map((f) => f.key)).toEqual(['bio', 'location', 'yearsOfExperience']);
  });
  ...
  it('preserves constant order when a middle field is present', () => {
    ...
    expect(missing.map((f) => f.key)).toEqual(['bio', 'yearsOfExperience']);
  });
});
