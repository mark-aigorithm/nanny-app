...
it('asks for a street address when the pin is set but the line is empty', () => {
  ...
  render(<RegistrationLocationScreen />);
...
it('labels itself step 4 of the mother’s 5', () => {
  render(<RegistrationLocationScreen />);
  expect(screen.getByText('STEP 4 OF 5 — LOCATION & PREFERENCES')).toBeTruthy();
});

it('starts with no preference chosen for her', () => {
  expect(useRegistrationDraftStore.getState().preferences).toEqual([]);
  render(<RegistrationLocationScreen />);
  expect(screen.getByText('Optional — pick any that apply.')).toBeTruthy();
});
